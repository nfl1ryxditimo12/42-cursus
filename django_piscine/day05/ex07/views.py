...
def update(request):
    try:
        ...
        if request.method == 'POST' and key:
            ...
            if form.is_valid() and key:
                try:
                    cur.execute(f"UPDATE ex07_movies SET opening_crawl='{title}' WHERE title='{key}'")
                    database.commit()
        ...
        try:
            cur.execute("""SELECT * FROM ex07_movies""")
    ...
    if response:
        return render(request, 'ex07/update.html', {'form': form, 'data': data})
    else:
        return HttpResponse('No data available')