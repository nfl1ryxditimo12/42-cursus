...
/*   Updated: 2021/06/18 22:03:37 by seonkim          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int			press_key(int key, t_game *game)
{
	...
	else if (key == KEY_W && check_wall(game, game->map.map[x - 1][y]))
		process_key(game, x - 1, y, game->img.up);
	else if (key == KEY_S && check_wall(game, game->map.map[x + 1][y]))
		process_key(game, x + 1, y, game->img.down);
	else if (key == KEY_A && check_wall(game, game->map.map[x][y - 1]))
		process_key(game, x, y - 1, game->img.left);
	else if (key == KEY_D && check_wall(game, game->map.map[x][y + 1]))
